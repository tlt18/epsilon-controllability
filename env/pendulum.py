...
class Pendulum(BaseEnv):
    ...
    def step(self, action):
        ...
        # self.state[0] = self.state[0] % (2*math.pi)
        # if  math.pi/2 < self.state[0] < math.pi:
        #     self.state[0] = self.state[0]- math.pi
        reward = 0
        self.step_count += 1
    ...
    def done(self):
        ...
        if self.state[0] < self.observation_space.low[0] and self.state[0] > self.observation_space.high[0]:
            return True
        else:
            return False
